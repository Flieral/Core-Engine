    ...
    switch (interactionType) {
      case 'liked':
        raccoon.liked(userId, contentId).then(() => {
          return cb('[liked] Successful: Content (' + contentId + ') By User (' + userId + ')')
        })
        break
      case 'disliked':
        raccoon.disliked(userId, contentId).then(() => {
          return cb('[disliked] Successful: Content (' + contentId + ') By User (' + userId + ')')
        })
        break
      case 'unliked':
        raccoon.unliked(userId, contentId).then(() => {
          return cb('[unliked] Successful: Content (' + contentId + ') By User (' + userId + ')')
        })
        break
      case 'undisliked':
        raccoon.undisliked(userId, contentId).then(() => {
          return cb('[undisliked] Successful: Content (' + contentId + ') By User (' + userId + ')')
        })
        break
      case 'reported':
        raccoon.reported(userId, contentId).then(() => {
          return cb('[reported] Successful: Content (' + contentId + ') By User (' + userId + ')')
        })
        break
      case 'shared':
        raccoon.shared(userId, contentId).then(() => {
          return cb('[shared] Successful: Content (' + contentId + ') By User (' + userId + ')')
        })
        break
    }
  }
